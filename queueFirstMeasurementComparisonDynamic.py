...
for env in experiment_result_set_dynamic.experiment_env:
    for exp_result in env.experiment_result:
        for measurement in exp_result.measurements:
            first_mean.append(measurement.queuefirstBatch)
...
rangeDayStart = 7
rangeDayEnd = 9
x = np.arange(0, len(first_mean))
y = np.asarray(first_mean)
colors = ['r' if rm == "Mesos" else 'b' for rm in rms]
dailyTicks = int(len(first_mean)/numberDays)
...
plt.tight_layout()
figure.savefig(os.path.join(input_dir,'queuefirstevolutiondynamic.pdf'), format='PDF')