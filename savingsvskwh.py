...
savingsSingleBar = ax1.bar(ind, policies_single_savings.values()+[0.01], width, color='#007dad')
savingsMultiBar = ax1.bar(ind+width, policies_multi_savings.values()+[0.01], width, color='#ec6200')
...
ax2 = ax1.twinx()
ax2.plot(ind+width,policies_single_kw.values()+[15.0], linestyle='--', color='#18b0ea', linewidth=3)
ax2.plot(ind+width,policies_multi_kw.values()+[14.0],linestyle='--', color='#ffa038', linewidth=3)
ax2.set_ylabel('KWh saved / shuttings')

plt.rc('legend',**{'fontsize':16})
ax1.legend((savingsSingleBar[0], savingsMultiBar[0]), ("Single", "Multi"))
plt.tight_layout()
#plt.show()
figure.savefig('encuentramemamona.pdf', format='PDF')